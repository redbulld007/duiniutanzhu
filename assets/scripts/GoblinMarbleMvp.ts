...
export class GoblinMarbleMvp extends Component {
  private root!: Node; private g!: Graphics; private guide!: Graphics; private hud!: Label; private tip!: Label; private home!: Label; private lobby!: LobbyLayer; private upgrades!: UpgradeLayer;
  ...
  private bounceBonus = 0; private scoreBonus = 0; private splitEvery = 5;
  ...
  update(dt: number) { this.resize(); if (this.flying) { this.ball.set(this.ballNode.position.x, this.ballNode.position.y); if (this.ballNode.active && this.ball.y < this.playBottom - this.ballR) this.ballNode.active=false; if (this.cloneNode.active && this.cloneNode.position.y < this.playBottom - this.ballR) this.cloneNode.active=false; if(!this.ballNode.active&&!this.cloneNode.active)this.finishShot(); } if (this.resolving) this.resolve(dt); this.draw(); }
  ...
  private resolve(dt:number) {
    ...
    if(this.levelWon){this.upgrades.show();this.tip.string='本关达成！选择一项强化继续闯关';return;}
    if(this.ballsLeft<=0){this.over=true;this.tip.string='弹珠用尽！轻触屏幕重试本关';return;}
  ...
  private async chooseUpgrade(kind:UpgradeKind){
    ...
    if(kind==='all'){this.applyUpgrade('bounce');this.applyUpgrade('combo');this.applyUpgrade('split');}else this.applyUpgrade(kind);
    this.upgrades.hide(); this.completeLevel();
  }
  private applyUpgrade(kind:Exclude<UpgradeKind,'all'>){if(kind==='bounce')this.bounceBonus=Math.min(.32,this.bounceBonus+.12);if(kind==='combo')this.scoreBonus=Math.min(4,this.scoreBonus+1);if(kind==='split')this.splitEvery=Math.max(2,this.splitEvery-1);}
  private completeLevel(){const clearedStage=this.stage;const reward=20+clearedStage*5;this.stage++;this.bestStage=Math.max(this.bestStage,this.stage);this.baseBalls=Math.min(12,this.baseBalls+1);this.ballsLeft=this.baseBalls;this.coins+=reward;sys.localStorage.setItem('cow-marble-best-stage',String(this.bestStage));sys.localStorage.setItem('cow-marble-coins',String(this.coins));GameTelemetry.track('level_complete', { stage: clearedStage, nextStage: this.stage, score: this.score, balls:this.baseBalls, coinsAwarded: reward, coinsTotal: this.coins });this.score=0;this.targetScore=70+this.stage*30;this.levelWon=false;this.board();this.tip.string=`过关！基础弹珠 +1，当前 ${this.baseBalls} 颗`;}
  private retryStage(){this.score=0;this.ballsLeft=this.baseBalls;this.levelWon=false;this.over=false;this.board();this.tip.string=`重新挑战第 ${this.stage} 关 · ${this.ballsLeft} 颗弹珠`;}
  ...
  private draw() {
    ...
    this.hud.string=`第 ${this.stage} 关   积分 ${this.score}/${this.targetScore}   弹珠 ${this.ballsLeft}/${this.baseBalls}   牛币 ${this.coins}`;this.home.node.active=!this.lobby.visible&&!this.upgrades.visible;
